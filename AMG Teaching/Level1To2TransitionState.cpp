...
{
	level = std::unique_ptr<Level>(new Level(SCOTLANDTOMEXICOTRANSITION, ID, false, true, false));	//Pass in the path to the level to be loaded, as well as the state ID of the current state.
	level->Load();
	return true;
}
